...
The chemist enters text only. After the AI Process Assistant routes the problem, it automatically reads the recommended workflow aloud using the browser's built-in `speechSynthesis` API, unless muted. It does not call a hosted Hugging Face endpoint or any external TTS API. Use the sidebar **Mute assistant voice** toggle to silence narration.
